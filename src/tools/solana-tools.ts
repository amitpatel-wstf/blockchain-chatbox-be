...
export const marketTools: any[] = [
  ...
  },
  {
    ...
    requiredParams: ['filters', 'sortBy', 'limit','chain'],
    dataSchema: 'table: token, volume, market cap, score',
    run: async ({ filters, sortBy, limit,chain="eth" }: { filters: any; sortBy: any; limit: number;chain?:string }) => {
      const res = await fetch('https://deep-index.moralis.io/api/v2.2/discovery/tokens', {
        ...
        body: JSON.stringify({ filters, sortBy, limit, chain })
      });
      return res.json();
    }
  }
